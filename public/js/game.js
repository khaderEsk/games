...
function arrival(horse, horseNo) {
    ...
    horseProp[horseNo].top = undefined; // تأكد من تعريف horseProp مسبقًا
    ...
    if (resultIcons.length == 3) {
        if (bethorse === "horse" + horseNo) {
            funds += amount; // زيادة الرصيد إذا ربح الحصان
        } else {
            funds -= amount; // نقص الرصيد إذا خسر الحصان
        }
        document.getElementById("funds").innerText = funds;

        // إرسال الرصيد المحدث إلى الخادم عبر body
        fetch('/update-funds', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content') // استخدام CSRF Token هنا
                },
                body: JSON.stringify({
                    funds: funds
